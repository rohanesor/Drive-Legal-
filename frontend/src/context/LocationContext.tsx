...
import type { GeoInfo, GPSCoords } from '../types';
...
export const LocationProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const updateLocationState = useCallback(async (coords: GPSCoords) => {
    try {
      ...
      console.log(
        '[Location Context] reverse geocode starting for coords:',
        coords,
      );
      const geo = await LocationService.reverseGeocode(
        coords.latitude,
        coords.longitude,
      );
      setGeoInfo(geo);
      ...
      console.warn(
        '[Location Context] Geocoding failed during update, using fallback coords overlay:',
        err,
      );
  ...
  const refreshLocation = useCallback(
    async (showPrompt = false) => {
      ...
      try {
        ...
        console.warn('[Location Context] Refresh location failed:', message);

        let errorType: LocationStatus = 'Location Timeout';
        let displayError = message || 'Location fetch failed';

        if (message === 'Permission Denied') {
          errorType = 'Permission Denied';
        } else if (message === 'GPS Disabled') {
        ...
        if (showPrompt) {
          let alertMessage =
            'Smart Jurisdiction Engine requires location permissions and active GPS sensors. Please ensure Location is enabled in system settings.';
          if (errorType === 'Location Timeout') {
            alertMessage =
              'Unable to acquire GPS signal. Please ensure you have clear sky visibility and active sensors.';
          } else if (errorType === 'GPS Disabled') {
            alertMessage =
              'System GPS sensors are disabled. Please enable Location in your quick settings panel.';
          } else if (errorType === 'Permission Denied') {
            alertMessage =
              'Location permission was denied. Please grant permission in your system settings.';
          }

          Alert.alert('Location Access Required', alertMessage, [
        ...
        try {
          ...
          if (cached) {
            console.info(
              '[Location Context] GPS failed, loading cached fallback coordinates',
            );
          ...
          console.warn(
            '[Location Context] Cache load failed, defaulting to manual state',
            cacheErr,
          );
          setManualLocation('TN');
        }
